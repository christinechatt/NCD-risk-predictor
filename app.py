...
def load_artefacts():
    ...
    return model, scaler, config['features'], name_cfg['display_names'], \
           name_cfg['name_map'], shap_explainer, lime_explainer

model, scaler, feature_columns, display_names, name_map, shap_explainer, lime_explainer = \
    None, None, None, None, None, None, None
try:
    model, scaler, feature_columns, display_names, name_map, \
    shap_explainer, lime_explainer = load_artefacts()
...
<div class="main-header">
    ...
    <p>Christiana Chatt Richards · 2023/A/DSC/0001 · Supervisor: Mr. Obaje Williams</p>
</div>
""", unsafe_allow_html=True)
...
# ─────────────────────────────────────────────
st.sidebar.image(
    "https://img.icons8.com/color/96/000000/caduceus.png", width=60)
st.sidebar.title("Lifestyle Assessment")
st.sidebar.markdown("Complete all sections below, then click **Predict Risk**.")

# Helper maps (must match Notebook 1 encoding exactly)
...
with st.sidebar.expander("📏 Body Measurements", expanded=True):
    ...
    st.metric("Calculated BMI", bmi,
              delta="Normal" if 18.5<=bmi<25 else
                    ("Overweight" if bmi<30 else
                    ("Underweight" if bmi<18.5 else "Obese")))

with st.sidebar.expander("🍽️ Diet & Nutrition"):
...
with st.sidebar.expander("😰 Stress & Mental Health"):
...
with st.sidebar.expander("🏥 Medical History"):
    ...
    health    = st.slider("Self-rated overall health (1=Poor, 5=Excellent)", 1, 5, 3)
...
# ── TAB 1: ABOUT ──────────────────────────────
with tab1:
    st.markdown('<p class="section-header">About This System</p>',
                unsafe_allow_html=True)
...
Nigeria currently lacks localised, gender-specific predictive tools for NCD risk. Most existing models are trained on Western populations and do not account for Nigeria-specific lifestyle factors such as cooking oil reuse practices, traditional dietary patterns, and the unique socio-cultural stressors faced by Nigerian women.
...
with tab2:
    if not predict_btn:
        st.info("👈 Complete the lifestyle assessment in the sidebar and click **Predict My Risk** to see your results.")

        st.markdown('<p class="section-header">How Risk Categories Are Defined</p>',
                    unsafe_allow_html=True)
        c1, c2, c3 = st.columns(3)
        with c1:
            st.markdown('<div class="risk-low"><h4>🟢 Low Risk</h4><p>Lifestyle patterns indicate a lower probability of developing NCDs in the near term. Preventive habits should be maintained and reinforced.</p></div>', unsafe_allow_html=True)
        with c2:
...
with tab3:
    if 'prediction' not in st.session_state:
        ...
        inp_df= st.session_state['input_df']
...
- Continue regular physical activity (aim for at least 150 minutes/week)
- Maintain a balanced diet rich in fruits, vegetables, and whole grains
- Prioritise consistent 7–9 hours of quality sleep
- Keep stress levels manageable through regular rest and social support

**Prevention reminders:**
- Schedule an NCD screening at least once every two years
- Monitor blood pressure and blood sugar annually if aged 35+
...
🍽️ **Diet**
- Reduce fried and processed food consumption to no more than 2×/week
- Increase fruit and vegetable intake to at least 5 servings per day
- Avoid reusing cooking oil — use fresh oil each time or switch to smaller quantities

🏃 **Physical Activity**
- Aim for at least 30 minutes of moderate activity on 5 days per week
- Reduce continuous sitting — take a 5-minute walk every hour
...
- Consider relaxation practices such as prayer, deep breathing, or light exercise
...
- If already diagnosed, ensure medication and treatment plan are current and being followed
- Request a comprehensive NCD risk assessment from your doctor

**Critical lifestyle changes:**

🍽️ **Diet (high priority)**
- Eliminate daily fried and processed food consumption immediately
- Switch to healthier cooking methods: boiling, steaming, or grilling
- Increase water intake; eliminate sugary drinks

🏃 **Physical Activity (high priority)**
- Begin with 15–20 minutes of walking daily and increase gradually
- Avoid prolonged sitting — set hourly movement reminders

⚖️ **Weight Management**
- Work with a nutritionist or healthcare provider on a safe weight reduction plan if BMI ≥ 30
- Small consistent changes (5–10% weight loss) significantly reduce NCD risk
...
- Federal Ministry of Health NCD prevention clinics
        ...
        if bmi_v >= 25:
            st.warning(f"⚠️ BMI of {bmi_v} is above the healthy range. Weight management is an important component of NCD risk reduction.")

        st.markdown('<div class="disclaimer">⚠️ These recommendations are generated based on the lifestyle data provided and are intended for informational and health literacy purposes only. They do not replace clinical diagnosis or medical advice from a qualified healthcare professional.</div>', unsafe_allow_html=True)
...
<div class="footer">
    ...
    Christiana Chatt Richards · Department of Data Science · Miva Open University, Abuja · 2025<br>
